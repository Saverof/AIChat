...
from utils.logger import (
    AppLogger,
)  # Импорт собственного логгера для отслеживания работы
...
class OpenRouterClient:
    ...
    def __init__(self):
        ...
        self.base_url = (
            os.getenv("BASE_URL") or "https://openrouter.ai/api/v1"
        )  # Базовый URL API

        # Проверка наличия API ключа - не выбрасываем исключение, так как ключ может быть установлен позже
        if not self.api_key:
            # Логирование предупреждения
            self.logger.warning(
                "OpenRouter API key not found in .env, will need to be set later"
            )
        ...
        # Загрузка списка доступных моделей при инициализации, если есть API ключ
        self.available_models = self.get_models() if self.api_key else []
    ...
    def get_balance(self):
        ...
        try:
            ...
            if data:
                ...
                return (
                    f"${(data.get('total_credits', 0)-data.get('total_usage', 0)):.2f}"
                )
            return "Ошибка"
        except Exception as e:
    ...
    def set_api_key(self, api_key):
        ...
        self.logger.info("API ключ успешно обновлен")
